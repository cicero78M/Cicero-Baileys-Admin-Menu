...
function resolveCanonicalFetchedAt(fetchedAt) {
...
export async function upsertInstaPost(data) {
  ...
  const canonicalFetchedAt = resolveCanonicalFetchedAt(fetched_at);
      ...
      SET client_id = EXCLUDED.client_id,
          ...
          source_type = CASE
            WHEN COALESCE(insta_post.source_type, 'cron_fetch') = 'manual_input'
              OR EXCLUDED.source_type = 'manual_input'
            THEN 'manual_input'
          ...
          fetched_at = COALESCE(GREATEST(insta_post.fetched_at, EXCLUDED.fetched_at), EXCLUDED.fetched_at, insta_post.fetched_at)`,
    [
      client_id,
      shortcode,
      caption,
...
export async function getShortcodesTodayByUsername(username) {
  ...
  const res = await query(
    `SELECT p.shortcode FROM insta_post p JOIN clients c ON c.client_id = p.client_id
     WHERE c.client_insta = $1 AND ${getInstagramCreatedAtJakartaDateSql('p.created_at')} = $2`,
    [username, `${yyyy}-${mm}-${dd}`]
  );
  return res.rows.map(r => r.shortcode);
}

...
export async function getRekapLikesByClient(
  ...
  const { rows } = await query(`
    ...
    SELECT
      ...
      u.title,
      u.nama,
      COALESCE((SELECT usa.username FROM user_social_accounts usa WHERE usa.user_id = u.user_id AND LOWER(usa.platform) = 'instagram' AND usa.is_active = TRUE ORDER BY usa.account_order ASC, usa.created_at ASC LIMIT 1), u.insta) AS username,
      u.divisi,
      u.exception,
      u.client_id,

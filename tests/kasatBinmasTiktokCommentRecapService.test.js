...
jest.unstable_mockModule('../src/handler/fetchabsensi/tiktok/absensiKomentarTiktok.js', () => ({
  normalizeUsername: (value) => String(value || '').replace(/^@/, '').trim().toLowerCase(),
  extractUsernamesFromComments: (comments = []) =>
    comments.map((comment) => comment?.username || comment?.user?.username).filter(Boolean),
}));
...
test('menampilkan nama Polres ketika tidak ada Kasat Binmas aktif', async () => {
  mockFindAllOrgClients.mockResolvedValue([
    ...
    { client_id: 'POLRESC', nama: 'Polres C', regional_id: 'JATIM' },
  ]);
  mockGetUsersByClient.mockResolvedValue([
    { user_id: '1', client_id: 'POLRESA', client_name: 'Polres A', jabatan: 'Kasat Binmas', nama: 'Alpha', tiktok: '@alpha' },
  ]);
  mockGetRekapKomentarByClient.mockResolvedValue([
  ...
  expect(summary).toContain('📋 *Absensi Engagement Kasat Binmas*');
  expect(summary).toContain('📱 Platform: TikTok');
  expect(summary).toContain('Total Polres jajaran: 3');
  ...
  expect(summary).toContain('POLRES C — Belum tersedia akun aktif Kasat Binmas');
});

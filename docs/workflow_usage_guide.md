...
- `oprrequest` → mengelola data user, rekap link harian, serta **Menu Manajemen Engagement** untuk absensi Likes Instagram/Komentar TikTok sesuai status aktif akun client. Jika `client_insta_status` dan `client_tiktok_status` sama-sama aktif, submenu engagement menambahkan **3️⃣ Fetch Post Engagement** untuk menjalankan fetch post+likes Instagram dan fetch post+komentar TikTok sekaligus. Laporan absensi engagement pada mode akumulasi kini dikelompokkan per satfung dengan sub-list **lengkap/kurang/belum**. Operator/Super Admin client dapat masuk langsung, sedangkan Admin WhatsApp wajib memilih client bertipe **org** sebelum masuk menu operator. Submenu **Absensi registrasi user** dan **Absensi update data username** berada di *Kelola User* dan seluruh submenu menampilkan instruksi **ketik back** untuk kembali. Detail pada `docs/wa_operator_request.md`.
- `userrequest` → **dinonaktifkan** dan tidak lagi tersedia pada bot.
- `dirrequest` → menu Direktorat untuk rekap data, absensi, dan pengambilan konten. Pada absensi likes direktorat, submenu *6️⃣ (simple)* kini menggunakan filter yang sama dengan menu *9️⃣*: hanya **user aktif**, scope **client direktorat terpilih** secara ketat (`client_id` persis), serta pengecualian **divisi/satfung** tertentu melalui `filterAttendanceUsers`. Submenu *1️⃣1️⃣ Absensi user web dashboard Direktorat/Bidang* tetap merespons normal setelah validasi scope client diperbaiki (menggunakan daftar `scopeClientIds` yang benar), sehingga balasan tidak lagi berhenti tanpa output.

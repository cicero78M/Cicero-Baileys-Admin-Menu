...
- **5️⃣0️⃣ Fetch likes IG manual (hari ini)** → menjalankan fetch likes Instagram khusus konten manual di hari berjalan (WIB) berdasarkan `client_id` yang dipilih pada sesi dirrequest. Secara canonical pipeline memakai `source_type=manual_input`; untuk kompatibilitas data lama, nilai legacy `manual_fetch` juga diperlakukan sebagai konten manual.
  - Pada tahap enrichment komentar Instagram (setelah likes), pagination komentar dijalankan tanpa batas halaman eksplisit (`maxPage=0`) agar seluruh komentar bisa terambil. Proses berhenti otomatis saat pagination habis (`has_more=false`/token kosong), token berulang, atau 4 halaman berturut-turut tidak menghasilkan username komentar (guard anti-loop).

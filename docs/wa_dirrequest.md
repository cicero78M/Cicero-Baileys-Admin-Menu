...
- Query harian helper `getShortcodesTodayByClient` dan `getShortcodesYesterdayByClient` memprioritaskan filter `(fetched_at AT TIME ZONE 'Asia/Jakarta')::date`, sehingga konten yang dipublikasikan lama tetapi baru difetch hari ini tetap masuk ke absensi hari operasional saat ini. Pada deployment lama yang belum memiliki kolom `fetched_at`, helper otomatis fallback ke filter `created_at` agar tetap kompatibel.
...
- Submenu **6️⃣ (Simple)** kini memakai filtering personil yang sama dengan
  ...
  **client direktorat terpilih** (normalisasi uppercase), lalu diterapkan
  pengecualian **divisi/satfung** tertentu melalui `filterAttendanceUsers`.
- Ringkasan angka `total/lengkap/kurang/belum/noUsername` pada output likes
  simple dihitung dari dataset filter yang sama, sehingga konsisten dengan
  hasil rekap lintas submenu absensi likes direktorat.
...
- Menu **3️⃣3️⃣ Absensi Kasatker** kini menyesuaikan jabatan, role query,
  dan header berdasarkan direktorat terpilih: untuk **DITINTELKAM** bot
  memakai role `ditintelkam`, mengabsen jabatan yang mengandung kata
  **Kasat Intel**/**Kasat Intelkam**, dan menampilkan header

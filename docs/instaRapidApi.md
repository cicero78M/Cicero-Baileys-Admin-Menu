...
- **4️⃣6️⃣ Input IG post manual** → memanggil helper `fetchSinglePostKhusus(link, clientId)` untuk mengambil detail post Instagram dari link, lalu menyimpan ke `insta_post_khusus` dan `insta_post`. Setelah post tersimpan, alur yang sama langsung melanjutkan `handleFetchLikesInstagram(..., { shortcodes: [shortcode], sourceType: "manual_input", enrichComments: false })` agar likes untuk post manual tersebut ikut tersinkron **tanpa fetch/enrichment komentar Instagram**. Pada alur manual ini, `created_at` diset ke waktu upload manual bot (format ISO string), sedangkan waktu publish asli platform disimpan di `original_created_at` (diambil dari `taken_at` jika tersedia). `insta_post.source_type` ditandai `manual_input`.
...
- **5️⃣2️⃣ Fetch komentar IG manual (hari ini)** → menjalankan fetch komentar Instagram khusus konten `manual_input` di hari berjalan (WIB) berdasarkan `client_id` sesi dirrequest. Pagination komentar memakai delay per halaman **1000ms**. Saat menyimpan komentar, sistem kini memastikan relasi parent pada `ig_ext_posts` tersedia (resolve via `post_id` atau `shortcode`) untuk mencegah kegagalan foreign key `ig_post_comments_post_id_fkey` pada data manual lama yang belum memiliki row parent di tabel ekstensi. Selain penyimpanan komentar, username komentator juga di-merge dan di-upsert ke tabel `insta_like` per shortcode (union dengan data existing) agar mekanisme absensi/rekap likes tetap sinkron dengan pipeline fetch likes.

...
export async function fetchDmHashtagsForUser(username) {
  ...
  const { rows } = await query(
    `SELECT p.caption_text FROM ig_ext_posts p JOIN ig_ext_users u ON u.user_id = p.user_id WHERE u.username = $1 AND (p.created_at AT TIME ZONE 'Asia/Jakarta')::date = $2::date`,

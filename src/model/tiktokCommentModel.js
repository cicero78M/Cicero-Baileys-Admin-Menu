...
export async function getRekapKomentarByClient(
  ...
  const { rows } = await query(
    ...
    SELECT
      ...
      u.title,
      u.nama,
      COALESCE((SELECT usa.username FROM user_social_accounts usa WHERE usa.user_id = u.user_id AND LOWER(usa.platform) = 'tiktok' AND usa.is_active = TRUE ORDER BY usa.account_order ASC, usa.created_at ASC LIMIT 1), u.tiktok) AS username,
      u.divisi,
    ...
    LEFT JOIN comment_counts cc
      ON (EXISTS (SELECT 1 FROM user_social_accounts usa WHERE usa.user_id = u.user_id AND LOWER(usa.platform) = 'tiktok' AND usa.is_active = TRUE AND lower(replace(trim(coalesce(usa.username, '')), '@', '')) = cc.username) OR lower(replace(trim(coalesce(u.tiktok, '')), '@', '')) = cc.username)
